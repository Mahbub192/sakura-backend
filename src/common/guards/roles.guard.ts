...
export class RolesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    const handlerRoles = this.reflector.get<RoleType[]>(
      ROLES_KEY,
      context.getHandler(),
    );
    const classRoles = this.reflector.get<RoleType[]>(
      ROLES_KEY,
      context.getClass(),
    );
    ...
    if (!user) {
      console.error(
        '[RolesGuard] User object is missing from request. Request:',
        {
          url: request.url,
    ...
    if (!user.role) {
      console.error(
        '[RolesGuard] User role is missing. User object:',
    ...
    const hasRole = requiredRoles.some((role) => {
      ...
      if (!roleMatch) {
        console.error(
          `[RolesGuard] Role mismatch: user.role="${userRole}" (type: ${typeof userRole}), required="${requiredRole}" (type: ${typeof requiredRole})`,
        );
      } else {
        console.log(
          `[RolesGuard] Role match: user.role="${userRole}" === required="${requiredRole}"`,
        );
      }
      return roleMatch;
    });
    ...
    if (!hasRole) {
      console.error(
        '[RolesGuard] Access denied. User does not have required role.',
      );
    }
